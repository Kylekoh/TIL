...
- 하나의 HTML파일을 기반으로 자바스크립트를 이용해 동적으로 화면의 컨텐츠를 바꾸는 방식의 웹 어플리케이션이다.
...
- 첫 로딩만 기다리면, 동적으로 바르게 렌더링 되기 때문에 사용자 경험(UX)이 좋다.
- 서버에게 요청하는 횟수가 훨씬 적기 때문에 서버의 부담이 덜하다.

### 단점

- 모든 스크립트 파일이 로드될 때까지 기다려야 한다.
- 검색엔진의 검색 봇이 크롤링을 하는데 어려움을 겪어 SEO에 문제가 생길 수 있다.
